                ...
                discounts.map((d) => (
                  <tr key={d._id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {d.category === 'all' ? t('discount.allCategories') : t(`room.${d.category}`)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {d.bedType ? `${d.bedType} ${t('room.beds')}` : t('discount.allBedTypes')}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {d.discountType === 'percentage' ? `${d.value}%` : `${new Intl.NumberFormat('vi-VN').format(d.value)} VND`}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      >
                        {d.isActive ? t('discount.isActive') : t('discount.inactive')}
                      </span>
                    </td>
          ...
          <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
            <div className="relative mx-auto p-5 border w-full max-w-sm shadow-lg rounded-xl bg-white max-h-[90vh] overflow-y-auto">
              <h3 className="text-lg font-bold text-gray-900 mb-4">
                {editingDiscount ? t('discount.editDiscount') : t('discount.addDiscount')}
              </h3>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, bedType: e.target.value ? Number(e.target.value) : undefined })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div className="flex items-center">
                  ...
                  />
                  <label className="ml-2 block text-sm text-gray-900">{t('discount.isActive')}</label>
                </div>
